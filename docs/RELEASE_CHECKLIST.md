...
- [x] 后端全量 pytest 通过；当前数量见 CI 自动报告。
- [x] Ruff `app tests` 全部通过。
- [x] strict mypy `app tests` 零错误。
- [x] 前端 Vitest 全量通过；当前数量见 CI 自动报告。
- [x] TypeScript strict 检查通过。
- [x] Vite 从当前源码完整生产构建通过。
- [x] Playwright 桌面与移动视口关键流程通过；当前数量见 CI 自动报告。
...
- [x] 正式打包默认拒绝脏工作区和未带精确 `v2.2.12` 标签的提交。
...
- [x] 本地候选安装器编译通过，内嵌 payload SHA-256 校验通过，产品版本为 2.2.12.0。
...
产物必须由 `scripts/package-desktop.ps1` 在干净的 `v2.2.12` 标签提交上生成：

- `NovelAgentStudio-Setup-2.2.12.exe`
- `NovelAgentStudio-Portable-2.2.12.zip`
- `SHA256SUMS.txt`
