...
- The supported v2.2.10 Windows installer and portable ZIP require Windows x64 plus Microsoft Edge or WebView2 Runtime. Release artifacts must pass packaged GUI lifecycle checks and carry build provenance.
- DeepSeek's official preset supports `deepseek-v4-flash` and `deepseek-v4-pro`. Legacy official profiles are migrated on startup; identically named aliases on custom gateways remain unchanged.
