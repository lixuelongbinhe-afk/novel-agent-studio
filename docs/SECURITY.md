...
- Frontend dependency audit temporarily ignores only `GHSA-qwww-vcr4-c8h2`: the advisory affects React Router RSC server actions, which this client-only local application does not use. It was rechecked on 2026-08-03; React Router 7.18.2 remains affected and no compatible fixed release is available. Review again by 2027-01-31 and remove the exception as soon as a compatible fix is published.
